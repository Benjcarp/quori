...
class CommentController extends AbstractController
{
    ...
    public function rate(Request $request, Comment $comment, int $score, EntityManagerInterface $em, VoteRepository $voteRepository) {
        ...
        // je verifie que le current user n'est pas le proprietaire de la comment
        if($currentUser !== $comment->getAuthor()) {
            ...
            if($vote) {
                // s'il avait aimer le comment et qu'il reclique sur le like c'est pour enlever son vote
                // s'il n'avait pas aimer le comment et qu'il reclique sur le dislike c'est pour enlever son vote
